...
class DDQN(object):
    ...
    def update(self):
        ...
        for sample in sampler:
            ...
            with torch.no_grad():
                batch_max_actions = self.act(batch_next_obs, eps=0.)
                target_y = batch_rewards.view(-1, 1) + self.gamma * \
                           (1. - batch_dones.view(-1, 1).long()) * self.target_policy(batch_next_obs).gather(1, batch_max_actions.view(-1, 1).long())
