...
def main(control_dict, agent_dict, env_dict):
    ...
    mq.put((0, agent.target_actor.state_dict()))
...
def eval(env_info, queue, agent_dict, env_params, control_params):
    ...
    agent = instantiate_network(agent_dict['network_name_actor'], **{"input_shape": agent_dict['observation_shape'],
                                                                     "output_shape": agent_dict['output_shape']})
    agent.eval()

    while True:
        episode, params = queue.get(timeout=100)
        if episode == "end":
            break
